    ...
    CSRState& csrState)
{
    ...
    auto crb = [&](size_t index)
        {
            constexpr std::string_view fields[] = { "lt", "gt", "eq", "so" };
            return fmt::format("{}.{}", cr(index / 4), fields[index % 4]);
        };
    ...
    case PPC_INST_ADDME:
        println("\t{}.s64 = {}.s64 + {}.ca - 1;", temp(), r(insn.operands[1]), xer());
        println("\t{}.ca = {}.u32 < {}.u32;", xer(), temp(), r(insn.operands[1]));
        println("\t{}.s64 = {}.s64;", r(insn.operands[0]), temp());
        if (strchr(insn.opcode->name, '.'))
            println("\t{}.compare<int32_t>({}.s32, 0, {});", cr(0), r(insn.operands[0]), xer());
    ...
    case PPC_INST_CROR:
        println("\t{} = {} | {};", crb(insn.operands[0]), crb(insn.operands[1]), crb(insn.operands[2]));
        break;
    ...
    case PPC_INST_DCBST:
        // Host memory is coherent for the recompiled program.
        break;
    ...
    case PPC_INST_FRSQRTE:
        printSetFlushMode(false);
        println("\t{}.f64 = float(1.0 / sqrt({}.f64));", f(insn.operands[0]), f(insn.operands[1]));
        break;
    ...
    case PPC_INST_MULHD:
        println("\t{}.s64 = int64_t((__int128({}.s64) * __int128({}.s64)) >> 64);", r(insn.operands[0]), r(insn.operands[1]), r(insn.operands[2]));
        break;
    ...
    case PPC_INST_SUBFZE:
        println("\t{}.u8 = ~{}.u32 + {}.ca < ~{}.u32;", temp(), r(insn.operands[1]), xer(), r(insn.operands[1]));
    ...
    case PPC_INST_VCFPUXWS128:
        printSetFlushMode(true);
        print("\tsimde_mm_store_si128((simde__m128i*){}.u32, simde_mm_cvttps_epi32(", v(insn.operands[0]));
        if (insn.operands[2] != 0)
            println("simde_mm_mul_ps(simde_mm_load_ps({}.f32), simde_mm_set1_ps({}))));", v(insn.operands[1]), 1u << insn.operands[2]);
        else
            println("simde_mm_load_ps({}.f32)));", v(insn.operands[1]));
    ...
    case PPC_INST_VCMPGTSW:
        println("\tsimde_mm_store_si128((simde__m128i*){}.u32, simde_mm_cmpgt_epi32(simde_mm_load_si128((simde__m128i*){}.s32), simde_mm_load_si128((simde__m128i*){}.s32)));", v(insn.operands[0]), v(insn.operands[1]), v(insn.operands[2]));
        if (strchr(insn.opcode->name, '.'))
            println("\t{}.setFromMask(simde_mm_load_ps({}.f32), 0xF);", cr(6), v(insn.operands[0]));
        break;
    ...
    case PPC_INST_VNOR128:
        println("\tsimde_mm_store_si128((simde__m128i*){}.u8, simde_mm_andnot_si128(simde_mm_or_si128(simde_mm_load_si128((simde__m128i*){}.u8), simde_mm_load_si128((simde__m128i*){}.u8)), simde_mm_cmpeq_epi32(simde_mm_setzero_si128(), simde_mm_setzero_si128())));", v(insn.operands[0]), v(insn.operands[1]), v(insn.operands[2]));
        break;
    ...
    case PPC_INST_VPKSHSS128:
        ...
        {
            println("\t{}.s32 = {}.s16[{}];", temp(), v(insn.operands[i < 8 ? 1 : 2]), 7 - (i % 8));
            println("\t{}.s8[{}] = {}.s32 > INT8_MAX ? INT8_MAX : {}.s32 < INT8_MIN ? INT8_MIN : {}.s32;", v(insn.operands[0]), 15 - i, temp(), temp(), temp());
        }
        break;
    ...
    case PPC_INST_VPKUHUS128:
        ...
        for (size_t i = 0; i < 16; i++)
        {
            println("\t{}.u32 = {}.u16[{}];", temp(), v(insn.operands[i < 8 ? 1 : 2]), 7 - (i % 8));
            println("\t{}.u8[{}] = {}.u32 > UINT8_MAX ? UINT8_MAX : {}.u32;", v(insn.operands[0]), 15 - i, temp(), temp());
        }
        break;
    ...
    case PPC_INST_VPKUWUM:
        ...
        for (size_t i = 0; i < 8; i++)
            println("\t{}.u16[{}] = {}.u32[{}] & 0xFFFF;", v(insn.operands[0]), 7 - i, v(insn.operands[i < 4 ? 1 : 2]), 3 - (i % 4));
        break;
    ...
    case PPC_INST_VPKSWSS128:
        ...
        for (size_t i = 0; i < 8; i++)
        {
            println("\t{}.s64 = {}.s32[{}];", temp(), v(insn.operands[i < 4 ? 1 : 2]), 3 - (i % 4));
            println("\t{}.s16[{}] = {}.s64 > INT16_MAX ? INT16_MAX : {}.s64 < INT16_MIN ? INT16_MIN : {}.s64;", v(insn.operands[0]), 7 - i, temp(), temp(), temp());
        }
        break;
    ...
    case PPC_INST_VPKSWUS128:
        ...
        {
            println("\t{}.s64 = {}.s32[{}];", temp(), v(insn.operands[i < 4 ? 1 : 2]), 3 - (i % 4));
            println("\t{}.u16[{}] = {}.s64 > UINT16_MAX ? UINT16_MAX : {}.s64 < 0 ? 0 : {}.s64;", v(insn.operands[0]), 7 - i, temp(), temp(), temp());
        }
        break;
    ...
    case PPC_INST_VRLH:
        // TODO: vectorize, ensure endianness is correct
        for (size_t i = 0; i < 8; i++)
            println("\t{}.u16[{}] = uint16_t(({}.u16[{}] << ({}.u8[{}] & 0xF)) | ({}.u16[{}] >> ((16 - ({}.u8[{}] & 0xF)) & 0xF)));", v(insn.operands[0]), i, v(insn.operands[1]), i, v(insn.operands[2]), i * 2, v(insn.operands[1]), i, v(insn.operands[2]), i * 2);
        break;
    ...
    case PPC_INST_VSUBSHS:
        ...
        for (size_t i = 0; i < 8; i++)
        {
            println("\t{}.s32 = int32_t({}.s16[{}]) - int32_t({}.s16[{}]);", temp(), v(insn.operands[1]), i, v(insn.operands[2]), i);
            println("\t{}.s16[{}] = {}.s32 > INT16_MAX ? INT16_MAX : {}.s32 < INT16_MIN ? INT16_MIN : {}.s32;", v(insn.operands[0]), i, temp(), temp(), temp());
        }
        break;
    ...
    case PPC_INST_VSUBUWS:
        for (size_t i = 0; i < 4; i++)
            println("\t{}.u32[{}] = {}.u32[{}] > {}.u32[{}] ? {}.u32[{}] - {}.u32[{}] : 0;", v(insn.operands[0]), i, v(insn.operands[1]), i, v(insn.operands[2]), i, v(insn.operands[1]), i, v(insn.operands[2]), i);
        break;
