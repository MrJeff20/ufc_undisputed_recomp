...
inline simde__m128i simde_mm_vsl(simde__m128i a, simde__m128i b)
{
    ...
    alignas(16) uint8_t shiftBytes[16];
    alignas(16) uint8_t dst[16] = {};
    simde_mm_storeu_si128((simde__m128i*)src, a);
    simde_mm_storeu_si128((simde__m128i*)shiftBytes, b);

    const uint8_t shift = shiftBytes[0] & 0x7;
    if (shift == 0)
        return a;

    // Vector bytes are stored reversed relative to the PowerPC big-endian order.
    for (size_t i = 0; i < 16; i++)
        dst[i] = uint8_t((src[i] << shift) | (i == 0 ? 0 : src[i - 1] >> (8 - shift)));
    return simde_mm_loadu_si128((simde__m128i*)dst);
}
